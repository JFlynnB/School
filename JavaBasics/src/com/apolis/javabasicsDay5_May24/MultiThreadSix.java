...
public class MultiThreadSix {
	...
	public static void main(String[] args) {
		...
		Thread thread01 = new Thread(new RunnableSix(array, 3, 4));
		Thread thread02 = new Thread(new RunnableSix(array, 2, 3));
		
		 thread01.start();
		 thread02.start(); 
		 
		 /* The basic functionality is achieved, but the output looks very messy. */
	}
...
class RunnableSix implements Runnable{
	...
	RunnableSix(int[] array, int x, int y){
		theArray = array;
		pos1 = x;
		pos2 = y;
	}
	...
	public static synchronized void swapElements(int position1, int position2, int[] arr) {
